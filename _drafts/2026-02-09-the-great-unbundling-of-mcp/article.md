...
And here's what that means in practice: a skill pointing at `gh`, a skill calling a REST endpoint, a skill backed by an MCP server — the agent performs about the same. The execution layer doesn't determine what the agent can do. It determines who maintains the infrastructure, who manages auth, and who gets paged when something breaks.
